// Lab: drain window and batch cap over a real Track A store (spec
// 2064/sqlo1 doc 04 section 7, milestone A2 lab 03).
//
// The shared queue's two constants, the dirty-bytes threshold that asks
// for a drain cycle (8 MiB) and the per-cycle op cap (1024), were set
// from the doc table against the placeholder store. This lab prices them
// against sqlo1a: the threshold sets how stale a record may sit in RAM
// before it is durable (drain lag) and how full a batch runs, the cap
// sets how long one ApplyBatch holds the store lock that point reads
// also need.
//
// The drainer itself is package-internal to engine/sqlo1 on purpose, so
// the harness mirrors its policy rather than importing it: a coalescing
...
// The writer loop runs its own drain cycles, matching the owner-loop
// shape where drain time is stolen from write time.
package main
...
func runAll(cfg config, out io.Writer) error {
	...
	path := filepath.Join(cfg.dir, "batchdrain.db")
	...
	// not-founds. Fixed batch size; the preload is not measured.
	var seq int64
	...
	// Writer loop with inline drain cycles: the owner-loop shape.
	q := newQueue(cfg.keys)
	...
	drainCycle := func() error {
		...
		drained += len(batch)
		batches++
		if wm := fileMB(path + "-wal"); wm > walPeak {
			walPeak = wm
		}
		return nil
	}
