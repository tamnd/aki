...
Since B3 the suite runs on both backends: -store a is the SQLite substrate below, -store b the same segmented-hash model over sqlo1b records, and the verdict reads from the arm that will actually carry the constant.
...
An oracle test at seg_max 512 pins the model on both arms through the store readback probes: fence partition, sorted in-range entries, under-threshold rows, honest fill classes, exact root count, and the exact reference map.
On the b arm roots are plain records under the user key, segments are subkey records under a rooth minted per hash key, a flush is one DrainBatch with the high-water sequence, and the checkpoint cadence calls the store's checkpoint; the W2/W4 WAL column stays modeled arithmetic, identical on both arms.
...
## Run
    ...
    ./run.sh            # both arms x {2016, 4032, 8064} x {small, med, large} x setpct {10, 50, 90}, gate box
    go run . -quick     # smoke (add -store b for the Track B arm)
    go test ./...       # smoke plus the segment oracle, both arms
