...
Pending: same-session interleaved A/B at run 3's LTM protocol, old binary (0082229) against this slice, SET uniform plus the GET uniform and GET zipf control cells, VmRSS sampled.

## Verdict

Pending the box A/B; the local sweep says log-direct placement plus batched appends, with the flush threshold at the shipped 1MiB.
