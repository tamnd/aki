#!/bin/bash
# f3 reactor A/B matrix (m10-pullforward slice 7).
# Three aki arms resident at once (goroutine-single 7111, goroutine-pair 7114,
# reactor 7115) plus redis 7112 and valkey 7113, all taskset 0-7; generator
# 8-15. Arms alternate within each rep (same-session interleaving) and every
# aki-bench invocation measures its arm AND both rivals, so per-arm ratios are
# same-invocation. Protocol per m0-run3: warm 3s, 3 timed 8s windows, none
# discarded, FLUSHALL between reps on all servers, 1M keys uniform,
# redis-benchmark --threads 4 with -P matching the pipeline. Resumable via
# $G/done.list.
set -u
export PATH=/root/bin:/usr/local/go/bin:/usr/bin:/bin:/usr/sbin:/sbin

G=/root/f3gate/reactor-ab/matrix
F3SRV=/root/f3gate/reactor-ab/bin/f3srv
AB=/root/f3gate/reactor-ab/bin/aki-bench
RB=/root/bin/redis-benchmark
CLI=/root/bin/redis-cli
SMASK=0-7
CMASK=8-15
PS=7111; PR=7112; PV=7113; PP=7114; PX=7115
NL=${NL:-3}   # reactor loops, lab 19's winner (the frozen 2/5-share default)
WARM=3s
KEYS=1000000

SPID=""; RPID=""; VPID=""; PPID2=""; XPID=""
DIR_S=""; DIR_R=""; DIR_V=""; DIR_P=""; DIR_X=""
CELL=""

log() { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] $*"; }
meta() { echo "$*" >> "$G/cells/$CELL.meta"; }

port_of() { case $1 in single) echo $PS;; pair) echo $PP;; reactor) echo $PX;; redis) echo $PR;; valkey) echo $PV;; esac; }
pid_of() { case $1 in single) echo $SPID;; pair) echo $PPID2;; reactor) echo $XPID;; redis) echo $RPID;; valkey) echo $VPID;; esac; }
...
flush_all() {
  local p
  for p in $PS $PP $PX $PR $PV; do $CLI -p $p flushall >/dev/null 2>&1; done
  sleep 0.5
}

rss_snap() { # tag
  meta "rss[$1] single=$(awk '/VmRSS/{print $2$3}' /proc/$SPID/status 2>/dev/null) pair=$(awk '/VmRSS/{print $2$3}' /proc/$PPID2/status 2>/dev/null) reactor=$(awk '/VmRSS/{print $2$3}' /proc/$XPID/status 2>/dev/null) redis=$(awk '/VmRSS/{print $2$3}' /proc/$RPID/status 2>/dev/null) valkey=$(awk '/VmRSS/{print $2$3}' /proc/$VPID/status 2>/dev/null)"
}

start_servers() { # $1 arena-mib
  local arena=$1
  DIR_S=$(mktemp -d $G/tmp/s.XXXXXX); DIR_P=$(mktemp -d $G/tmp/p.XXXXXX)
  DIR_X=$(mktemp -d $G/tmp/x.XXXXXX); DIR_R=$(mktemp -d $G/tmp/r.XXXXXX)
  DIR_V=$(mktemp -d $G/tmp/v.XXXXXX)
  taskset -c $SMASK $F3SRV --addr 127.0.0.1:$PS --arena-mib $arena >$DIR_S/f3srv.log 2>&1 &
  SPID=$!
  taskset -c $SMASK $F3SRV --addr 127.0.0.1:$PP --arena-mib $arena -conn-shape pair >$DIR_P/f3srv.log 2>&1 &
  PPID2=$!
  ...
  VPID=$!
  local p
  for p in $PS $PP $PX $PR $PV; do
    wait_ping $p || { meta "FATAL port $p did not answer PING"; return 1; }
  done
  meta "pins single=$(taskset -cp $SPID 2>&1) reactor=$(taskset -cp $XPID 2>&1) redis=$(taskset -cp $RPID 2>&1)"
  meta "launch arena_mib=$arena net_loops=$NL warm=$WARM io_threads=4"
  meta "driver_stamp single=$($CLI -p $PS info 2>/dev/null | grep -o 'net_driver:[a-z]*') pair=$($CLI -p $PP info 2>/dev/null | grep -o 'net_driver:[a-z]*') reactor=$($CLI -p $PX info 2>/dev/null | grep -o 'net_driver:[a-z]*')"
  rss_snap launch
  return 0
}
...
stop_servers() {
  local p
  for p in $SPID $PPID2 $XPID $RPID $VPID; do kill $p 2>/dev/null; done
  for p in $SPID $PPID2 $XPID $RPID $VPID; do wait $p 2>/dev/null; done
  for p in $PS $PP $PX $PR $PV; do port_closed_wait $p; done
  local arm dir
  for arm in S P X; do
    eval dir=\$DIR_$arm
  ...
  rm -rf "$DIR_S" "$DIR_P" "$DIR_X" "$DIR_R" "$DIR_V"
  SPID=""; PPID2=""; XPID=""; RPID=""; VPID=""
}
...
matrix_cell() {
  ...
  local r arm
  for r in 0 1 2; do
    for arm in single pair reactor; do
      flush_all
  ...
  local rbwl=$wl
  for arm in single pair reactor redis valkey; do
    flush_all
  ...
  meta "dbsize_end single=$($CLI -p $PS dbsize) reactor=$($CLI -p $PX dbsize) redis=$($CLI -p $PR dbsize)"
  stop_servers
}
