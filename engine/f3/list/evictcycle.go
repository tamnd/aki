...
// The maxmemory evictor's list arm (spec 2064/f3/16 sections 6.4 and 7.3): the
...
// through the one store.EvictScore comparator, so a list victim weighs against a
...
// Go's randomized map iteration is the sampler.

// EvictVictim samples up to sample keys from this shard's list registry and returns
...
func EvictVictim(cx *shard.Ctx, policy uint8, sample int, volatileOnly bool) (key []byte, score int64, ok bool) {
	...
	for k, l := range g.m {
		...
		if volatileOnly && l.expireAt == 0 {
			continue
		}
		seen++
		idle := store.IdleSecondsFrom(l.clock, cx.NowMs)
		sc := store.EvictScore(policy, uint32(idle), l.expireAt)
...
// EvictKey drops the list at key and reports whether one was there, logging the
...
// EvictAccounted reports whether this shard's list registry tracks resident bytes
