...
// The maxmemory evictor's zset arm (spec 2064/f3/16 sections 6.4 and 7.3): the
...
// through the one store.EvictScore comparator, so a zset victim weighs against a
// string, a set, or a stream on one scale, and Go's randomized map iteration is
// the sampler the way ReapExpired samples the keyspace.

// EvictVictim samples up to sample keys from this shard's zset registry and returns
...
func EvictVictim(cx *shard.Ctx, policy uint8, sample int, volatileOnly bool) (key []byte, score int64, ok bool) {
	...
	for k, z := range g.m {
		...
		if volatileOnly && z.expireAt == 0 {
			continue
		}
		seen++
		idle := store.IdleSecondsFrom(z.clock, cx.NowMs)
		sc := store.EvictScore(policy, uint32(idle), z.expireAt)
...
// EvictKey drops the zset at key and reports whether one was there, logging the
...
// EvictAccounted reports whether this shard's zset registry tracks resident bytes
