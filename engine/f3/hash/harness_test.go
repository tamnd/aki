...
// The command-level harness: the real hash handlers on a shard runtime, driven
// the same way the list slice drives its handlers. It exercises routing, reply
...
// wantArray compares an array reply against want, where a nil element is written
// as the sentinel below so a nil-in-array can be asserted positionally.
const nilElem = "\x00nil\x00"
...
func wantArray(t *testing.T, raw []byte, want ...string) {
	...
	for i := range want {
		if want[i] == nilElem {
			if arr[i] != nil {
				t.Fatalf("array[%d] = %v, want nil", i, arr[i])
			}
			continue
		}
