...
// The maxmemory evictor's hash arm (spec 2064/f3/16 sections 6.4 and 7.3): the
...
// through the one store.EvictScore comparator, so a hash victim weighs against a
// string, a set, or a stream on one scale. The registry hangs off the shared regs
// map keyed by the shard's store (the same seam ReapExpired reaches through), and
// Go's randomized map iteration is the sampler. The scoring reads the whole-key
// access clock and deadline; a per-field HEXPIRE TTL is not a key the evictor
// sheds, so it never enters the victim pick.

// EvictVictim samples up to sample keys from this shard's hash registry and returns
...
func EvictVictim(cx *shard.Ctx, policy uint8, sample int, volatileOnly bool) (key []byte, score int64, ok bool) {
	...
	for k, h := range g.m {
		...
		if volatileOnly && h.expireAt == 0 {
			continue
		}
		seen++
		idle := store.IdleSecondsFrom(h.clock, cx.NowMs)
		sc := store.EvictScore(policy, uint32(idle), h.expireAt)
...
// EvictKey drops the hash at key and reports whether one was there, logging the
...
// EvictAccounted reports whether this shard's hash registry tracks resident bytes
