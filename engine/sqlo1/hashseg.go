...
// refuses the split.
func decodeHashSeg(p []byte, enc hashEnc) (hashSeg, error) {
	...
	it := hashEntryIter{p: s.entries, enc: enc}
	seen := 0
	minExp := int64(0)
	var prevFH uint64
	var prevField []byte
...
func hashSegSet(dst []byte, s hashSeg, fh uint64, field, val []byte, expMs, nowMs int64) (out []byte, created, revived bool, err error) {
	...
	for {
		...
		if !placed {
			if efh == fh && bytes.Equal(f, field) {
				out = appendHashEntry(out, field, val, expMs, s.enc)
				placed = true
				created = false
...
func parseHashSegEntries(dst []hashSegEntry, region []byte, enc hashEnc) ([]hashSegEntry, error) {
	dst = dst[:0]
...
func appendHashSegPayload(dst []byte, entries []hashSegEntry, enc hashEnc) []byte {
	out := grow(dst, hashSegHdrLen)
	minExp := int64(0)
	for _, e := range entries {
		out = appendHashEntry(out, e.field, e.val, e.expMs, enc)
...
func mergeHashSegs(dst, lop, hip []byte, enc hashEnc) ([]byte, error) {
	los, err := decodeHashSeg(lop, enc)
	if err != nil {
		return nil, err
	}
	his, err := decodeHashSeg(hip, enc)
	if err != nil {
		return nil, err
	}
	if los.n > 0 && his.n > 0 {
		var lastFH uint64
		var lastField []byte
		it := hashEntryIter{p: los.entries, enc: enc}
		for {
		...
		it = hashEntryIter{p: his.entries, enc: enc}
