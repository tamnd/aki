...
// ParseCommand parses one client command from the front of buf and returns
// the argument list and the number of bytes consumed. The arguments alias
// buf and are valid until the caller reuses it. A multibulk request is the
// normal path; a line not starting with '*' is an inline command. An empty
// inline line is consumed and returns zero arguments, which the caller
// skips.
func ParseCommand(buf []byte) (args [][]byte, n int, err error) {
	...
	if count > maxArgs {
		return nil, 0, protoErrf("invalid multibulk length")
	}
	...
	for range count {
		...
		if buf[p] != '$' {
			return nil, 0, protoErrf("expected '$', got '%c'", buf[p])
		}
		...
		if buf[q+blen] != '\r' || buf[q+blen+1] != '\n' {
			return nil, 0, protoErrf("invalid bulk terminator")
		}
...
func parseInline(buf []byte) (args [][]byte, n int, err error) {
	end := -1
	for i, c := range buf {
		if c == '\n' {
			end = i
		...
		if i >= maxInlineLen {
			return nil, 0, protoErrf("too big inline request")
		}
	}
	if end < 0 {
