...
// TestZRangeStoreTornTail cuts the WAL after every frame of a store
// cadence and demands the destination be all-or-nothing: at every
// prefix it holds exactly one of the images a completed ZRANGESTORE
// left behind (or nothing), never a blend, with the two families
// agreeing on whichever image survived. The shrunk caps push the wide
// store through the paged bulk build, and the cadence walks the dest
// across inline, paged, back to inline, and deleted.
func TestZRangeStoreTornTail(t *testing.T) {
	...
	// so the wide store pages the dest fence under the shrunk caps.
	...
	// snapshot reads the dest image the live runtime holds.
	...
	store := func(src string, lo, hi, wantN int64) {
		t.Helper()
		n, err := z.ZRangeStore(ctx, dst, []byte(src), lo, hi)
		if err != nil || n != wantN {
			t.Fatalf("ZRangeStore(%s, %d, %d) = (%d, %v), want %d", src, lo, hi, n, err, wantN)
		}
		flush()
		images = append(images, snapshot(z))
	}
	store("srcS", 0, 5, 5)   // inline dest
	store("srcF", 0, 40, 40) // paged dest
	if paged, err := z.FencePagedForTest(ctx, dst); err != nil || !paged {
		t.Fatalf("wide store did not page the dest fence (paged %v, err %v)", paged, err)
	}
	store("srcS", 3, 9, 6)     // back to inline over the paged plane
	store("srcF", 100, 200, 0) // empty window deletes
...
// TestZPopTornTail cuts the WAL after every frame of a pop cadence
// and demands each pop be all-or-nothing: at every prefix the board
// holds exactly one of the command-boundary images, never a state
// with some of a pop's removals applied and others not, with the two
// families agreeing on whichever image survived. The shrunk caps and
// fat members push the multi-delete groups through run deaths and the
// paged fence ladder, and the cadence covers inline pops, paged pops
// from both ends, a draining pop that retires the plane behind a
// genbump, and a rebirth.
func TestZPopTornTail(t *testing.T) {
	restore := sqlo1.SetZFenceCapsForTest(2, 4, 2, 6)
	defer restore()
	ctx := context.Background()
	dir := t.TempDir()
	...
	fat := func(i int) string {
		return fmt.Sprintf("f%03d:%s", i, strings.Repeat("z", 680))
	}
	...
	pop := func(count int64, maxSide bool, wantN int) {
		t.Helper()
		n := 0
		err := z.ZPopCount(ctx, key, count, maxSide, func(int64) {}, func(float64, []byte) { n++ })
		if err != nil {
			t.Fatalf("ZPopCount(%d, max=%v): %v", count, maxSide, err)
		}
		if n != wantN {
			t.Fatalf("ZPopCount(%d, max=%v) popped %d, want %d", count, maxSide, n, wantN)
		}
		images = append(images, snapshot(z))
	}

	// Phase 1: inline board, inline pops from both ends, inline death.
	for i := range 5 {
	...
	// Phase 2: fat rebirth through the paged transition, then pops
	// whose delete groups sweep whole runs and fence pages.
	for i := range 36 {
		zadd(fat(i), float64(i%5))
	}
	flush()
	...
	// Phase 3: the draining pop retires the plane behind a genbump.
	pop(50, true, 18)
	flush()
	...
	for i := range 4 {
		zadd(fmt.Sprintf("r%02d", i), float64(i)+0.5)
	}
	flush()
	pop(10, false, 4)
	flush()
	...
	for n := 0; n <= len(cap.frames); n++ {
		...
		if !found {
			t.Fatalf("cut %d: board holds %d members matching no command boundary (a torn pop)", n, len(visible))
		}
		...
		if n == len(cap.frames) && len(visible) != 0 {
			t.Fatalf("full tail: board holds %d members, the last pop drained it", len(visible))
		}
		if err := db2.Close(); err != nil {
			t.Fatalf("cut %d: close: %v", n, err)
		}
