...
// A throwaway RESP client for TestEncodingAgainstRedis: it speaks just enough of
// the protocol to send an inline-array command and read one flat reply
// (status, error, integer, or bulk). It never runs unless AKI_REDIS_ADDR is set,
// so it stays out of the default test path and adds no dependency.
