...
// hash exists here at all. A live root with no TTL answers (0, true).
// Owner goroutine only.
func Deadline(cx *shard.Ctx, key []byte) (int64, bool) {
	h := registry(cx).m[string(key)]
	if h == nil {
		return 0, false
	}
...
// reports whether a hash exists to carry it. Owner goroutine only.
