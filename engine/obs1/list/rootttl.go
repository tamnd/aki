...
// Deadline reports key's root deadline in absolute unix ms and whether a
// list exists here at all. A live root with no TTL answers (0, true).
// Owner goroutine only.
func Deadline(cx *shard.Ctx, key []byte) (int64, bool) {
	l := registry(cx).m[string(key)]
	if l == nil {
		return 0, false
	}
...
// reports whether a list exists to carry it. Owner goroutine only.
