...
// The command-level harness: the real list handlers on a shard runtime, driven
// the same way the set donation oracle drives its handlers. It lets the tests
// exercise routing, reply framing, and the registry the way a client would,
// without a socket.
...
const (
	...
	opLpos
	opObject
	opSet // seed a string key to test WRONGTYPE and OBJECT fallthrough
	opLast
)
...
// decodeReply turns a raw RESP2 reply into the recursive form the live client
// uses: nil for null, a string for status/integer/bulk, an []any for an array,
// an errReply for an error line.
...
func wantArray(t *testing.T, raw []byte, want ...string) {
	t.Helper()
	got := decodeReply(t, raw)
	arr, ok := got.([]any)
	if !ok {
		t.Fatalf("reply = %v, want array %v", render(got), want)
	}
	if len(arr) != len(want) {
		t.Fatalf("array len %d, want %d (%v)", len(arr), len(want), render(got))
	}
	for i := range want {
		if s, ok := arr[i].(string); !ok || s != want[i] {
			t.Fatalf("array[%d] = %v, want %q", i, arr[i], want[i])
		}
	}
}

func wantEmptyArray(t *testing.T, raw []byte) {
	t.Helper()
	got := decodeReply(t, raw)
	arr, ok := got.([]any)
	if !ok || len(arr) != 0 {
		t.Fatalf("reply = %v, want empty array", render(got))
	}
}
